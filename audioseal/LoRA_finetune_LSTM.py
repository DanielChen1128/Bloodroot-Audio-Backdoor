...
class LoRALinear(nn.Module):
    def __init__(self, base_weight: torch.Tensor, r: int, alpha: float):
        super().__init__()
        out_dim, in_dim = base_weight.shape
        self.r = int(r)
        self.scaling = float(alpha) / max(1, self.r)
        self.register_buffer("base_weight", base_weight, persistent=False)
        self.A = nn.Parameter(torch.zeros(self.r, in_dim))
        self.B = nn.Parameter(torch.zeros(out_dim, self.r))
        nn.init.kaiming_uniform_(self.A, a=math.sqrt(5))
        nn.init.zeros_(self.B)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (..., in_dim)
        W = self.base_weight + (self.B @ self.A) * self.scaling  # (out_dim, in_dim)
        return x @ W.t()


class LoRALSTM(nn.Module):

    def __init__(self, lstm: nn.LSTM, r: int, alpha: float):
        super().__init__()
        assert isinstance(lstm, nn.LSTM) and lstm.num_layers in (1, 2, 3, 4)
        self.base = lstm
        for p in self.base.parameters():
            p.requires_grad_(False)
        ...
        # 建立 LoRA 化的「線性映射」包裝
        self.lora_ih = nn.ModuleList()
        self.lora_hh = nn.ModuleList()
        for layer in range(self.base.num_layers):
            Wi = getattr(self.base, f"weight_ih_l{layer}")
            Wh = getattr(self.base, f"weight_hh_l{layer}")
            self.lora_ih.append(LoRALinear(Wi.detach(), r=self.r, alpha=self.alpha))
            self.lora_hh.append(LoRALinear(Wh.detach(), r=self.r, alpha=self.alpha))

    def forward(self, x, hx=None):
        seq_len, batch, input_size = x.shape
        h_t, c_t = (hx if hx is not None else (None, None))
        if h_t is None:
            h_t = x.new_zeros(self.base.num_layers, batch, self.base.hidden_size)
        if c_t is None:
            c_t = x.new_zeros(self.base.num_layers, batch, self.base.hidden_size)
        ...
        for t in range(seq_len):
            ...
            h_cur = []
            c_cur = []
            for l in range(self.base.num_layers):
                Wi = getattr(self.base, f"weight_ih_l{l}")
                Wh = getattr(self.base, f"weight_hh_l{l}")
                bi = getattr(self.base, f"bias_ih_l{l}")
                bh = getattr(self.base, f"bias_hh_l{l}")
                gates = self.lora_ih[l](inp) + self.lora_hh[l](h_t[l]) + bi + bh
            ...
            h_t = torch.stack(h_cur, 0)
            c_t = torch.stack(c_cur, 0)
            outputs.append(inp)

        y = torch.stack(outputs, 0)
        return y, (h_t, c_t)
...
def unfreeze_output_bias(model: nn.Module, name="decoder.model.15.conv.conv.bias"):
    try:
        p = dict(model.named_parameters())[name]
        p.requires_grad_(True)
        LOG.info("[Bias] unfrozen: %s", name)
    except KeyError:
        LOG.warning("[Bias] not found: %s", name)
...
def inject_lora_into_decoder(
    ...
    for fullname, mod in list(model.named_modules()):
        if not _hit(fullname):
            continue

        wrap = None
        if isinstance(mod, _CONV1D_TYPES):
            wrap = LoRAConv1d(mod, r=rank, alpha=alpha)
        elif isinstance(mod, _CONVT_TYPES):
            wrap = LoRAConvTranspose1d(mod, r=rank, alpha=alpha)
        elif include_lstm and isinstance(mod, nn.LSTM):  
            wrap = LoRALSTM(mod, r=rank, alpha=alpha)  
        ...
        rep_str = f"{fullname} ({type(mod).__name__} → {type(wrap).__name__})"
        replaced.append(rep_str)
        if trace:
            LOG.info("  - %s", rep_str)
...
class PairNpyDataset(Dataset):
    ...
    def _rand_crop_pair(self, x: torch.Tensor, w: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if L <= self.seg_len:
            if L < self.seg_len:
                ...
                x = torch.nn.functional.pad(x, (0, pad))
                w = torch.nn.functional.pad(w, (0, pad))
            return x[:self.seg_len], w[:self.seg_len]
    
        start = random.randint(0, L - self.seg_len)
        return x[start:start + self.seg_len], w[start:start + self.seg_len]

    def __getitem__(self, idx: int):
        rp, wp = self.pairs[idx]
        x = self._load(rp)
        w = self._load(wp)
        x, w = self._rand_crop_pair(x, w)  # ✅ 同起點裁切
...
class Trainer:
    def __init__(self, args):
        ...
        if getattr(self.args, "tune_out_bias", 0):
            unfreeze_output_bias(self.G)
        ...
        # 5) Optim & AMP
        # 收集參數
        lora_params = [p for n, p in self.G.named_parameters() if p.requires_grad and "lora_" in n]
        bias_params = [p for n, p in self.G.named_parameters() if p.requires_grad and "bias" in n and "lora_" not in n]
        other_params = [p for n, p in self.G.named_parameters() if p.requires_grad and ("lora_" not in n) and ("bias" not in n)]
        
        # 推薦：LoRA lr = 3x，其他維持
        base_lr = args.lr
        self.opt = torch.optim.Adam([
            {"params": lora_params, "lr": base_lr * 3.0},
            {"params": bias_params, "lr": base_lr},
    ...
    def train(self):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            meter = {"sup": 0.0, "stft": 0.0,
                     "mel": 0.0, "amp": 0.0, "mix":0.0, "tot": 0.0}
            count = 0
            ...
            for step, (x, wm5) in enumerate(pbar):
                ...
                with torch.cuda.amp.autocast(enabled=self.use_fp16):
                    ...
                    loss_mix = self.l_mix(y_hat, y_wm5)
                ...
                pbar.set_postfix({
                    ...
                    "amp": f"{meter['amp']/count:.4f}",
                    "mix": f"{meter['mix']/count:.4f}"
                })

            # ---- epoch ----
            avg = {k: (v / max(1, count)) for k, v in meter.items()}
            LOG.info(
                "[Epoch %d/%d] train: L=%.4f | sup=%.4f | stft=%.4f | mel=%.4f | amp=%.6f | mix=%.4f"  ,
                epoch, args.epochs,
                avg["tot"], avg["sup"], avg["stft"], avg["mel"], avg["amp"], avg["mix"]
            )
...
def parse_args():
    ...
    ap.add_argument("--tune-out-bias", type=int, default=0,
                help="Unfreeze final output conv bias (1/0)")
    ...
    ap.add_argument("--lambda-mix", type=float, default=2)
